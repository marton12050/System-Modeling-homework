...
import hu.bme.mit.inf.symod.fm592.homework.chessclock.ChessClockStatemachine;
...
public class ReflectiveTimeInterfaceProvider {
	...
	public static Class<?> iTimer() {
		try {
			return Class.forName("hu.bme.mit.inf.symod.fm592.homework.ITimer");
	...
	public static Class<?> iTimerCallback() {
		if(isTimerAvailable()) {
			try {
				return Class.forName("hu.bme.mit.inf.symod.fm592.homework.ITimerCallback");
			} catch (ClassNotFoundException e) {
				//e.printStackTrace();
				return null;
			}
	...
	public static Object providePhysicsTimer() {
		if(isTimerAvailable()) {
			try {
				return Class.forName("hu.bme.mit.inf.symod.fm592.homework.TimerService").newInstance();
